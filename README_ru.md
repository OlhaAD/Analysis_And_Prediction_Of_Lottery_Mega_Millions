# Анализ и прогнозирование частоты выпадения номеров в лотерее "Mega Millions" с использованием анализа трендов и полиномиальной регрессии

## Как запустить проект
1. Склонируйте репозиторий на свой локальный компьютер.
2. Данные для анализа находятся в папке /data.
3. Откройте ноутбуки в папке /notebooks через Google Colab или Jupyter Notebook для просмотра и выполнения кода.
4. Установите необходимые зависимости с помощью команды pip install -r requirements.txt (предполагается, что вы создали файл requirements.txt со списком всех библиотек).

## Цели и задачи
### Цели
Основная цель этого исследовательского проекта заключается в анализе исторических данных о лотерее для прогнозирования частоты выпадения номеров в будущем с использованием такой модели машинного обучения, как полиномиальная регрессия. Проект нацелен на предоставление всестороннего сравнения между традиционным анализом трендов и методами машинного обучения, чтобы определить наиболее эффективные методы прогнозирования результатов лотереи.
...
3. **Анализ данных**
  ...
  - Проведение анализа для статистической проверки выпадения двух последовательных номеров.
...
- **Scikit-learn:** Библиотека для машинного обучения в Python. Использовалась для предобработки данных и оценки моделей.
  ...
  - mean_absolute_error, mean_squared_error, r2_score: Метрики оценки качества моделей.
  - LinearRegression: Модель линейной регрессии.
  - PolynomialFeatures: Преобразование данных для полиномиальной регрессии.
...
**Нормализация и масштабирование данных для полиномиальной регрессии**
...
#### Проведение анализа для статистической проверки выпадения двух последовательных номеров
В данном проекте был проведен анализ исторических данных о лотерее для выявления различных закономерностей. Один из аспектов анализа заключался в исследовании вероятности выпадения двух последовательных номеров. Для этого были выполнены следующие шаги:

```python
# Функция для расчета процента строк с последовательными номерами
def calculate_consecutive_percent(df):
    # Конвертируем столбцы с номерами в числовой формат
    cols = ['Number1', 'Number2', 'Number3', 'Number4', 'Number5']
    df[cols] = df[cols].apply(pd.to_numeric, errors='coerce')
    # Функция для проверки наличия последовательных номеров
    def has_consecutive_numbers(row):
        numbers = sorted(row.dropna().values)  # Сортируем и обрабатываем NaN
    ...
    # Рассчитываем процент строк с последовательными номерами
    percentage = df['has_consecutive'].mean() * 100
    return percentage
# Применяем функцию к каждому датафрейму и выводим результаты
...
Результаты анализа показали, что процент строк с последовательными номерами составил:
